...
| 2026-02-20 | self | Multi-hunk `apply_patch` failed on diverged files and delayed edits. | Use smaller targeted patches after inspecting exact line context when files are actively changing. |
| 2026-02-20 | self | While resolving type warnings in a drifted file, a status icon branch in chat briefly regressed to wrong JSX content. | Re-open and sanity-check the exact edited block immediately after any non-trivial patch in high-churn files. |
...
- If unexpected workspace changes are present and user says continue, proceed on top of current state.
...
- For unstable files, patch small sections with exact context instead of large multi-hunk edits.
